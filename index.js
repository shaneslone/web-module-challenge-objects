...
const hotdog = createMenuItem('Hot Dog', 2, 'Lunch');
const pancakes = createMenuItem('Pancakes', 10, 'Breakfast');
const pizza = createMenuItem('Pizza', 15, 'Lunch');
...
reviews.push({
  name: 'Shane',
  rating: 2,
  feedback: "I wasn't a big fan of the place.",
});

/* Task 5: Add the following feedback to Reyna's rating - "this place is chill with really cool people, great for getting work done on weekdays"*/

reviews[7].feedback =
  'his place is chill with really cool people, great for getting work done on weekdays';
...
*/
function getLastReview(objectArr) {
  return `${objectArr[objectArr.length - 1].name} gave the restaurant a ${
    objectArr[objectArr.length - 1].rating
  } and their feedback was: ${objectArr[objectArr.length - 1].feedback}`;
}
...
function getReviewByRating(objectArr, rating) {
  let tempArr = [];
  for (let i = 0; i < objectArr.length; i++) {
    if (objectArr[i].rating >= rating && objectArr[i].rating < rating + 1) {
...
*/
function getLongReviews(objectArr) {
  let outputArr = [];
  for (let i = 0; i < objectArr.length; i++) {
    if (objectArr[i].feedback.split(' ').length > 15) {
      outputArr.push(objectArr[i]);
    }
  }
  return outputArr;
}
